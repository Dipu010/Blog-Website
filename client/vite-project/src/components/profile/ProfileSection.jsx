      ...
      <div className="realtive bg-gray-300 w-screen h-screen overflow-x-hidden pt-[70px]">
        <div className="relative mx-auto w-[1000px] bg-gray-500 h-[180px] text-white text-xl  rounded-sm">
          <div className=" absolute w-[200px] h-[200px] rounded-full bg-white z-10 flex justify-center items-center mt-[75px] ml-[30px]">
            <div className=" w-[190px] h-[190px] rounded-full bg-blue-500  z-10"><img src={data.profilePicture} className=" w-[190px] h-[190px] rounded-full bg-blue-500  z-10" alt="" /></div>
          </div>
        </div>
        ...
        <div className=' w-[1000px]  bg-gray-700 h-[130vh] text-gray-50 mx-auto mt-5 pt-[70px] pl-[50px]'>
          ...
          <div className='flex justify-between items-center'>
            <div className='flex flex-col'>
              <p className='text-bold text-2xl rounded-md font-bold mt-2 text-gray-50'>Activites</p>
              <p className=' text-blue-500'>500 followers</p>
            </div>
            <div className='flex gap-[-1]'>
              <button className=' px-4 py-1 rounded-full text-xl border-2 mr-8 bg-blue-500 text-black' onClick={() => navigate('/blog')}> Create a Post </button>
              <IoIosCreate className=' text-4xl' />
            </div>
          </div>
          <div className=' flex gap-3 mt-5'>
            <button className={`${show == 1 ? ' bg-green-500' : 'bg-transparent'} px-4 rounded-full border-2 border-white text-[18px]`} onClick={() => setShow(1)} > Posts</button>
            <button className={` ${show == 2 ? 'bg-green-500' : 'bg-transparent'} px-4 rounded-full border-2 border-white text-[18px]`} onClick={() => setShow(2)} > Comments</button>
          </div>
          <BlogStructure />
